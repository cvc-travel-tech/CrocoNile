...
<form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
    ...
    <h5 class="title-bar no-border-bottom">
      ...
      <h5 class="title-bar no-border-bottom">
        Program Name : {{ $data->package->name }}
    </h5>
...
<ul>
    <li>{{$acoms->d_from}} / {{$acoms->d_to}} /{{$acoms->hotel}} /{{$acoms->location}} / {{$acoms->notes}}</li>
       ...
       <div class="booking-history-manager">
        ...
        <div class="accomidation_details">
        <h6 style="color:#ec991f;">Itinerary</h6>
    <ul>
...
<li>{{$value['date_t'] ." " ." ". $value['type'] ." from " .  $value['from_t']  ." " . " to " . $value['to_t'] .", via " . $value['arrive_via'] . " at " . $value['arrive_at']  ." | receiver name". $value['receiver_name'] 
...
<div class="accomidation_details" style="margin-top:20px;">
        <h6 style="color:#ec991f;">complimentary Item</h6>
        </div>
        @foreach($comp as $c)
<ul>
    <li>{{$c->name_c}} /{{$c->description_c}}</li>
        ...
        <br>
                ...
                <h6 style="color:#ec991f;">Included Sightseeing:</h6>
...
<br>
<br>
        <h6 style="color:#ec991f;"> Domestic Flight Passanger Number : {{$data->num_dom }} </h6>
    ...
    <div class="accomidation_details"  style="margin-top:20px;">
        ...
        @foreach($roomdata as $room)
              ...
              Name: {{$room->gender}}  {{$room->first_name}} {{$room->last_name}}   |  {{$room->phone}}  |   {{$room->nationality}}  | 
               @if($room->room_type ==1)
                single
...
<hr>
    ...
    <ul>
     @foreach($data->booking_package_options as $key => $value)
         <li>{{$value->package_option->name}} / {{$value->no_of_pax}}  /  {{$value->price}} $ per-pex</li>
